      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-body">
              <form role="form" action="{{ route('setups.fee.amount.update',$feeAmounts[0]->fee_category_id) }}" method="POST" id="MyForm">
              @csrf
                <div class="add_item"> 
                  <div class="form-row"> 
                      ...
                      <select name="fee_category_id" class="form-control select2bs4">
                        <option value="">Select Fee Category</option>
                          @foreach($feeCategories as $feeCategory)
                            <option value="{{ $feeCategory->id }}" {{($feeAmounts[0]->fee_category_id == $feeCategory->id)?"selected":""}}>{{ $feeCategory->name }}</option>
                          @endforeach
                      </select>              
                    </div>                  
                  </div>
                          ...
                          <select name="class_id[]" class="form-control select2bs4">
                            <option value="">Select Class</option>
                              @foreach($classes as $class)
                                <option value="{{ $class->id }}" {{($feeAmount->class_id == $class->id)?"selected":""}}>{{ $class->name }}</option>
                              @endforeach
                          </select>                 
                        </div>                
                        <div class="form-group col-md-5">
                          <label for="amount">Amount</label>
                          <input type="number" name="amount[]" class="form-control" placeholder="Enter Amount" value="{{ $feeAmount->amount }}">                 
                        </div>                    
                ...
                <button type="submit" class="btn btn-primary">Update Fee Amount</button>  
              </form> 
            </div>                     
          </div> 
        </div>        
